...
CREATE POLICY test_creator_batches_admin ON public.test_creator_batches FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS internal_creator_flags_admin ON public.internal_creator_flags;
CREATE POLICY internal_creator_flags_admin ON public.internal_creator_flags FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS campaign_test_assignments_admin ON public.campaign_test_assignments;
CREATE POLICY campaign_test_assignments_admin ON public.campaign_test_assignments FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS automation_logs_admin ON public.automation_logs;
CREATE POLICY automation_logs_admin ON public.automation_logs FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));

DROP POLICY IF EXISTS automation_generation_jobs_admin ON public.automation_generation_jobs;
CREATE POLICY automation_generation_jobs_admin ON public.automation_generation_jobs FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin')) WITH CHECK (public.has_role(auth.uid(), 'admin'));
...
DECLARE i int := 0; s text := '';
BEGIN
  FOR i IN 1..p_len LOOP s := s || (floor(random() * 10))::text; END LOOP;
  RETURN s;
END; $$;
...
DECLARE
  ...
  i int; out text := '';
BEGIN
  FOR i IN 1..p_len LOOP out := out || substr(alphabet, (floor(random() * length(alphabet)))::int + 1, 1); END LOOP;
  RETURN out;
END; $$;
...
RETURNS text[] LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
  SELECT CASE
    ...
    ELSE coalesce(array(
      SELECT 'IC-' || lpad(nextval('public.creator_public_ic_seq'::regclass)::text, 5, '0')
...
BEGIN
  ...
  VALUES ('process_scheduled_test_submissions', ok_count, err_count,
...
CREATE OR REPLACE FUNCTION public.sync_creator_leaderboard_points_delta(p_user uuid, p_delta_pts numeric)
...
BEGIN
  FOR r IN
    SELECT s.id AS sid, s.creator_id AS cid, s.campaign_id,
           s.manual_views, s.sim_view_cap, s.created_at AS sub_created,
           c.payout_per_1m_views, c.max_earnings_per_post, c.budget_remaining,
           e.id AS earning_id, e.amount AS earning_amt
  ...
  INSERT INTO public.automation_logs (job_name, items_processed, errors_count, duration_ms, details)
  VALUES ('grow_test_creator_views', upd, err_count,
    (extract(epoch FROM (clock_timestamp() - t0)) * 1000)::int,
...
ALTER PUBLICATION supabase_realtime ADD TABLE public.automation_generation_jobs;
